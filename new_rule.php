...
//Create the form
$new_rule_form = new individual_settings_form_new_rule('new_rule.php?userid='.$USER->id.'&categoryid='.$categoryid, array('rule_id' => $ruleid, 'categoryid' => $categoryid, 'weightingdesc' => $weighting_description));     
...
if ($fromform = $new_rule_form->get_data()) {
    ...
    if(isset($fromform->submit_get_rule_description)) {
        redirect(new moodle_url('new_rule.php', array('userid' => $USER->id, 'categoryid' => $categoryid, 'ruleid' => $fromform->rule_id)));
    }
